...
class TrainingPlannerApp:
    def __init__(self, root):
        ...
        self.type_combobox = ttk.Combobox(root, textvariable=self.type_var, values=["Кардио", "Силовая", "Растяжка", "Йога"], state="readonly")
        ...
        ttk.Button(root, text="Применить фильтры", command=self.apply_filters).grid(row=7, column=0, columnspan=2, pady=5)
    ...
    def validate_input(self):
        ...
        date_str = self.date_entry.get().strip()
         ...
         try:
            ...
            messagebox.showerror("Ошибка", f"Дата должна быть в формате {DATE_FORMAT} (например: 2026-05-02)")
            return False
    ...
    def add_training(self):
         """Добавление новой тренировки в таблицу и список."""
    ...
    def run(self):
         """Запуск приложения и заполнение таблицы при старте."""
         for training in self.trainings:
             self.tree.insert("", "end", values=(training["date"], training["type"], training["duration"]))
         
         # Привязка события закрытия окна для сохранения данных
